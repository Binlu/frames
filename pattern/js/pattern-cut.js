...
!function(g){
	...
	var setPattern=function(opt){
		...
		this.mask_ele=this.getEle(".js-mask-pic",this.pattern_area)[0];							//花型区域
		this.pattern_ele=this.getEle(".js-pattern-pic",this.pattern_area)[0];					//模板图片
		...
		this.canvas_ele3=document.createElement('canvas');											//副画布
	...
	setPattern.prototype={
		...
		setPosFunc:function(){			//初始化值
			var a=this,d=a.def,p=a.pattern_area;
			d.line_pos_top=a.line_ele.offsetTop;
		...
		bindFunc:function(ntype){			//事件绑定
			var a=this;
			var b=a.btn,d=a.def,p=a.pattern_area,is_start_ratio=false,ry=0;
			var timer=null;
			setStartSty();
			if(ntype==2){return};
			...
			function setZoomFunc(){
				...
				if(typeof _y ==="number"){
					...
					p.style["background-size"]=d.ratio*d.width+"px";
				}
			};
